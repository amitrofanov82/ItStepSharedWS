package lesson40.swing2.notebook.ui.console;
...
public class NotebookCommunicator {
	...
	private int askOperation() {
		while (true) {
			...
			print("7 - Сохранить записи в файл");
			print("8 - Восстановить записи из файла");
	...
	private void operationCredentialRecord(int numberOperation) {
		...
		case 7:
			try {
				print(notebook.saveCredentialRecords());
			} catch (IOException e) {
				e.printStackTrace();
			}
			break;
		case 8:
			try {
				print(notebook.restoreCredentialRecords());
			} catch (IOException e) {
				e.printStackTrace();
			}
			break;
