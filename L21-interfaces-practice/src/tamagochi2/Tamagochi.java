package tamagochi2;
///придраться в общем не к чему. Но в будущем подобные классы не должны будут содержать
///аут.принтлн. А на данном этапе, пока мы не загоняемся обработкой ошибок, пожалуй хорошая практика 
///использовать ерр.принтлн как реакцию на неверные данные. 
///+ 1 маленький коммент по методу alive()
public class Tamagochi {
    ...
    private static final int DOG = 1; ///Стоит обратить внимание, что подобное одинаковое значение констант, отвечающих за непересекающиеся множества значений - это в общем нормально. Но иногда может оказаться полезно по одной лишь цифре определять о чем идет речь.  Цифр много, повторяться нет нужды. Хотя иногда и так удобнее. Это не плюс и не минус. 
    private static final int CAT = 2;
    private static final int HORSE = 3;
    private static final int ELEPHANT = 4;
    ...
    public boolean alive() {///обычно называют подобные методы isAlive()
	if (health > 0) {
	    return true;
	}
	return false;
